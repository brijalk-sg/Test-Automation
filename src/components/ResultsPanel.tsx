...
function stripImports(code: string): string {
    return code
        ...
        .filter(line => {
            ...
            return !trimmed.startsWith('import ') && !trimmed.startsWith('expect.extend(');
        })
        .join('\n')
        .replace(/^\n+/, '');
}
...
function mergeImports(tests: TestSuggestion[], componentName: string): string {
    ...
    for (const line of lines) {
        ...
        const named = trimmed.match(/^import\s+\{([^}]+)\}\s+from\s+['"]([^'"]+)['"]/);
        if (named) {
            const module = named[2];
            const exports = named[1].split(',').map(s => s.trim()).filter(Boolean);
        ...
        const both = trimmed.match(/^import\s+(\w+)\s*,\s*\{([^}]+)\}\s+from\s+['"]([^'"]+)['"]/);
        if (both) {
            defaultImports[both[3]] = both[1];
            const module = both[3];
            const exports = both[2].split(',').map(s => s.trim()).filter(Boolean);
            if (!namedImports[module]) namedImports[module] = new Set();
            exports.forEach(e => namedImports[module].add(e));
        }
    }
    ...
    if (!namedImports['@testing-library/react']) namedImports['@testing-library/react'] = new Set();
    ['render', 'screen', 'fireEvent', 'waitFor'].forEach(e => namedImports['@testing-library/react'].add(e));
...
export const ResultsPanel: React.FC<ResultsPanelProps> = ({
    ...
    const allTests = [...results.unitTests, ...results.edgeCases, ...results.a11yTests];
                    ...
                    <div className="setup-steps">
                        <div className="setup-step">
                            ...
                            <div>
                                ...
                                <p className="step-desc">Run this in your project's root directory:</p>
                                <code className="step-code">npm install --save-dev jest ts-jest jest-environment-jsdom @testing-library/react @testing-library/jest-dom @testing-library/user-event @types/jest jest-axe @types/jest-axe</code>
                            </div>
                        </div>
                        <div className="setup-step">
                            <span className="step-number">2</span>
                            <div>
                                <p className="step-heading">Create <code>jest.config.ts</code> in project root</p>
  ...
  setupFilesAfterEnv: ['@testing-library/jest-dom/jest-globals'],
                        ...
                        <div className="setup-step">
                            ...
                            <div>
                                <p className="step-heading">Add test script to <code>package.json</code></p>
  ...
  "test": "jest"
                        ...
                        <div className="setup-step">
                            ...
                            <div>
                                ...
                                <p className="step-desc">Paste your React component code into the editor on the left, enter your Claude API key, and click <strong>Generate Tests</strong>. The AI will analyze your component and produce unit tests, edge cases, and accessibility tests.</p>
                            </div>
                        </div>
                        <div className="setup-step">
                            <span className="step-number">5</span>
                            <div>
                                <p className="step-heading">Download &amp; place the test file</p>
                                <p className="step-desc">Click <strong>"Download .test.tsx"</strong> above. Place the downloaded file next to your component:</p>
    ...
    ${name}.test.tsx     ← downloaded test file (place here)`}</pre>
                        ...
                        <div className="setup-step">
                            ...
                            <div>
                                ...
                                <p className="step-desc" style={{marginTop: '8px'}}>Or run all tests: <code>npm test</code></p>
                            </div>
                        </div>
                    </div>
                    <div className="setup-notes">
                        <p className="setup-note"><strong>Next.js:</strong> Use <code>next/jest</code> instead of the jest.config above — it handles transforms automatically.</p>
                        <p className="setup-note"><strong>Vite + Vitest:</strong> If you prefer Vitest, replace Jest with <code>npm i -D vitest @testing-library/react</code> and use <code>npx vitest</code>. The generated test code is compatible with both.</p>
                        <p className="setup-note"><strong>Tip:</strong> Some AI-generated tests may need minor adjustments based on your component's actual behavior. Review and tweak as needed.</p>
                    </div>
                </div>
            )}
